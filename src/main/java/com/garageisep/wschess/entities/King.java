...
public class King extends Piece {
    ...
    public List<Position> getPossibleMoves(Position from, GameSystem game) {
        ...
        for (int i = 0; i < 8; i++) {
            for (int j = 0; j < 8; j++) {
                if (!game.hasFriendlyPieceAt(i, j) && // No friendly piece
    ...
    private boolean[] canCastle(Position pos, GameSystem game) {
        ...
        if (game.getPieceAt(0, j) instanceof Rook rook && !rook.hasMoved()) {
        ...
        if (game.getPieceAt(7, j) instanceof Rook rook && !rook.hasMoved()) {
